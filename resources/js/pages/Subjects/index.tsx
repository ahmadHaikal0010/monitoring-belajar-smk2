            ...
            <div className="flex flex-col gap-6 p-6">
                <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
                    <div>
                        ...
                        <p className="text-muted-foreground">
                            Kelola materi dan mata pelajaran yang tersedia dalam sistem.
                        </p>
                    </div>
                                ...
                                <DropdownMenuContent align="end" className="w-48">
                                    ...
                                    <DropdownMenuItem onClick={() => handleSort('subjects.title')}>
                                        <div className="flex items-center justify-between w-full">
                                            ...
                                            {filters.sort === 'subjects.title' && (
                                                filters.direction === 'asc' ? <ArrowUp className="h-3 w-3" /> : <ArrowDown className="h-3 w-3" />
                                            )}
                                        </div>
                                    </DropdownMenuItem>
                                    ...
                                    <DropdownMenuItem onClick={() => handleSort('subjects.created_at')}>
                                        <div className="flex items-center justify-between w-full">
                                            <span>Tanggal Dibuat</span>
                                            {filters.sort === 'subjects.created_at' && (
                                                filters.direction === 'asc' ? <ArrowUp className="h-3 w-3" /> : <ArrowDown className="h-3 w-3" />
                                            )}
                                        </div>
                                    </DropdownMenuItem>
                                    ...
                                    <DropdownMenuItem
                                        onClick={() => {
                                            setSearch('');
                                            router.get('/teacher/subjects', {}, { replace: true });
                                        }}
                ...
                {subjects.data.length > 0 ? (
                    <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                        <AnimatePresence mode="popLayout">
                            {subjects.data.map((subject, index) => (
                                ...
                                >
                                    <Card className="group h-full flex flex-col overflow-hidden border-none bg-card/50 shadow-lg backdrop-blur-sm transition-all hover:shadow-xl hover:translate-y-[-2px]">
                                            ...
                                            <div className="flex items-start justify-between gap-4">
                                                ...
                                                <DropdownMenu>
                                                    ...
                                                    <DropdownMenuContent align="end" className="w-40">
                                                        ...
                                                        {(auth.user.role === 'admin' || (auth.user.role === 'guru' && auth.user.id === subject.teacher_id)) && (
                                                            <>
                                                                <DropdownMenuItem asChild>
                                        ...
                                        <CardFooter className="flex flex-col items-start gap-4 pt-4 border-t border-zinc-100 dark:border-zinc-800 bg-muted/20">
                    ...
                    <Card className="flex flex-col items-center justify-center p-12 text-center border-dashed border-2 bg-muted/20">
                        <div className="flex h-16 w-16 items-center justify-center rounded-full bg-zinc-100 dark:bg-zinc-800 mb-4 text-muted-foreground">
                            <BookOpen className="h-8 w-8" />
                        </div>
                        <h3 className="text-xl font-bold mb-2">Mata Pelajaran Kosong</h3>
                        <p className="text-muted-foreground max-w-xs mb-6">
                            Belum ada mata pelajaran yang terdaftar. Mulai dengan menambahkan mata pelajaran baru.
                        </p>
                        {auth.user.role === 'guru' ? (
                            <Button asChild>
                                <Link href="/teacher/subjects/create">
                ...
                {subjects.total > subjects.data.length && (
                    <div className="flex items-center justify-between border-t border-zinc-200 bg-muted/10 p-4 rounded-xl dark:border-zinc-800">
                        <p className="text-xs text-muted-foreground">
                            Menampilkan <span className="font-bold text-foreground">{subjects.from || 0}</span> sampai{' '}
                            <span className="font-bold text-foreground">{subjects.to || 0}</span> dari{' '}
                            <span className="font-bold text-foreground">{subjects.total}</span> mapel
                        </p>
