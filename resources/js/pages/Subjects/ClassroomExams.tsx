...
export default function ClassroomExams({
    subject,
    classroom,
    exams,
    otherClassroomExams,
    ...
    useEffect(() => {
        setLayoutProps({
            breadcrumbs: [
                ...
                { title: subject.title, href: `/teacher/subjects/${subject.id}?tab=exams` },
                { title: `Ujian Kelas ${classroom.name}`, href: '#' },
            ],
        });
    ...
    const handleCopySubmit = (e: React.FormEvent) => {
        ...
        router.post(
            `/teacher/subjects/${subject.id}/classrooms/${classroom.id}/exams/copy`,
            { exam_ids: selectedCopyIds },
            {
                onSuccess: () => {
    ...
    return (
        <>
            <Head title={`Ujian ${subject.title} - ${classroom.name}`} />
                ...
                <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                    <div className="flex items-center gap-4">
                        <Button variant="outline" size="icon" asChild>
                            <Link href={`/teacher/subjects/${subject.id}?tab=exams`}>
                        ...
                        <div>
                            ...
                            <h1 className="text-2xl font-bold tracking-tight sm:text-3xl">
                                Ujian & Evaluasi - {classroom.name}
                            </h1>
                            <p className="text-xs text-muted-foreground sm:text-sm">
                                Pengelolaan paket ujian dan evaluasi khusus kelas {classroom.name} ({subject.title}).
                            </p>
                        </div>
                    </div>
                    ...
                    {!isAdmin && (
                        <div className="flex items-center gap-2">
                            ...
                            <Button size="sm" className="h-9 gap-1.5 shadow-md shadow-primary/20 text-xs" asChild>
                                <Link href={`/teacher/exams/create?subject_id=${subject.id}&classroom_id=${classroom.id}`}>
                        ...
                        <table className="w-full border-collapse text-left text-sm">
                            ...
                            <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
                                {filteredExams.length > 0 ? (
                                            ...
                                            <td className="p-4 text-xs">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <span className="flex items-center gap-1 font-medium">
                                                        <Award className="h-3.5 w-3.5 text-amber-500" /> KKM: {exam.pass_score}
                                                    </span>
                                                </div>
                                            </td>
                                            <td className="p-4 text-xs">
                                                <div className="flex items-center gap-1">
                                                    ...
                                                    <span className="font-medium">{exam.questions_count ?? 0} Soal</span>
                                                </div>
                                            </td>
                                            {!isAdmin && (
                                                            ...
                                                            <DropdownMenuContent align="end" className="w-44">
                                                                <DropdownMenuItem asChild>
                                                                    <Link href={`/teacher/exams/${exam.id}`}>
                                                                        <Eye className="mr-2 h-4 w-4 text-primary" /> Bank Soal & Detail
                                                                    </Link>
                                                                </DropdownMenuItem>
                                                                <DropdownMenuItem asChild>
                                                                    <Link href={`/teacher/exams/${exam.id}/edit`}>
                                                                        <Pencil className="mr-2 h-4 w-4 text-primary" /> Edit Ujian
                                                                    </Link>
                                                                </DropdownMenuItem>
                                                                <DropdownMenuItem
                                                                ...
                                                                >
                                                                    <Trash2 className="mr-2 h-4 w-4" /> Hapus Ujian
                                                                </DropdownMenuItem>
                                                            </DropdownMenuContent>
                                                        </DropdownMenu>
                                                    </div>
                                    ...
                                    <tr>
                                        <td colSpan={isAdmin ? 4 : 5} className="p-12 text-center text-muted-foreground italic">
                                            {searchQuery || statusFilter !== 'all'
                                                ? 'Tidak ada ujian yang sesuai dengan pencarian/filter.'
                                                : 'Belum ada ujian khusus untuk kelas ini.'}
                                        </td>
                                    </tr>
                                )}
                            </tbody>
            ...
            {/* Modal Salin Ujian dari Kelas Lain */}
            <Dialog open={isCopyModalOpen} onOpenChange={setIsCopyModalOpen}>
                <DialogContent className="sm:max-w-lg">
                    <DialogHeader>
                        <DialogTitle className="flex items-center gap-2">
                            <Copy className="h-5 w-5 text-primary" />
                            Salin Ujian dari Kelas Lain
                        </DialogTitle>
                        <DialogDescription>
                            Pilih ujian beserta seluruh bank soal dari kelas lain untuk disalin ke kelas <strong>{classroom.name}</strong>.
                        </DialogDescription>
                    </DialogHeader>

                    <form onSubmit={handleCopySubmit} className="space-y-4 pt-2">
                        <div className="max-h-72 overflow-y-auto space-y-2 pr-1">
                            {otherClassroomExams.map((e) => {
                                const isSelected = selectedCopyIds.includes(e.id);

                                return (
                                    <div
                                        key={e.id}
                                        onClick={() => toggleCopySelect(e.id)}
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-[10px] text-muted-foreground line-clamp-1 mt-0.5">
                                                    Durasi: {e.duration} menit • KKM: {e.pass_score}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <DialogFooter className="pt-4 border-t border-zinc-100 dark:border-zinc-800">
                            ...
                            >
                                Salin {selectedCopyIds.length > 0 ? `${selectedCopyIds.length} ` : ''}Ujian Sekarang
                            </Button>
                        </DialogFooter>
                    </form>
                </DialogContent>
            ...
            <Dialog open={!!examToDelete} onOpenChange={(open) => !open && setExamToDelete(null)}>
                <DialogContent className="sm:max-w-[425px]">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Apakah Anda yakin ingin menghapus ujian <span className="font-bold text-foreground">{examToDelete?.title}</span>? Tindakan ini permanen.
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter className="mt-4 gap-2 sm:gap-0">
                        <Button variant="ghost" onClick={() => setExamToDelete(null)} disabled={isDeleting}>
                            Batal
                        </Button>
                        <Button variant="destructive" onClick={handleDeleteExam} disabled={isDeleting} className="gap-2">
