...
export default function AssignmentIndex({ assignments, subjects, selectedSubject, filters, mode }: Props) {
    ...
    setLayoutProps({
        breadcrumbs: [
            ...
            ...(mode === 'assignments' && selectedSubject ? [{
                title: selectedSubject.title,
                href: `/teacher/assignments?subject_id=${selectedSubject.id}`,
            }] : []),
        ],
    });
    ...
    const handleSubjectSelect = (subjectId: string) => {
        router.get('/teacher/assignments', { ...filters, subject_id: subjectId, page: 1, search: '' });
    };
    ...
    return (
        <>
            <Head title={mode === 'subjects' ? "Pilih Mata Pelajaran - Tugas" : `Tugas: ${selectedSubject?.title}`} />
                ...
                <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
                    <div>
                        <div className="flex items-center gap-3">
                            {mode === 'assignments' && (
                                <Button variant="ghost" size="icon" className="h-10 w-10" onClick={() => router.get('/teacher/assignments')}>
                                    <ArrowLeft className="h-5 w-5" />
                                </Button>
                            )}
                            <h1 className="text-3xl font-bold tracking-tight">
                                {mode === 'subjects' ? "Manajemen Tugas Siswa" : selectedSubject?.title}
                            </h1>
                        </div>
                        <p className="text-muted-foreground mt-1">
                            {mode === 'subjects' 
                                ? "Pilih mata pelajaran untuk mengelola atau membuat tugas siswa." 
                                : `Daftar tugas siswa untuk mata pelajaran ${selectedSubject?.title}.`}
                        </p>
                    </div>
                    <div className="flex flex-col items-center gap-3 sm:flex-row">
                        <div className="flex w-full items-center gap-3 sm:w-auto">
                            <div className="relative w-full md:w-64">
                                ...
                                <Input
                                    placeholder={mode === 'subjects' ? "Cari mata pelajaran..." : "Cari tugas..."}
                            ...
                            <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end" className="w-48">
                                        ...
                                        <>
                                            ...
                                            <DropdownMenuItem onClick={() => handleSort('due_date')}>
                                                <div className="flex w-full items-center justify-between">
                                                    ...
                                                    {filters?.sort === 'due_date' && (filters?.direction === 'asc' ? <ArrowUp className="h-3 w-3" /> : <ArrowDown className="h-3 w-3" />)}
                                                </div>
                                            </DropdownMenuItem>
                                            <DropdownMenuItem onClick={() => handleSort('status')}>
                                                <div className="flex w-full items-center justify-between">
                                                    <span>Status</span>
                                                    {filters?.sort === 'status' && (filters?.direction === 'asc' ? <ArrowUp className="h-3 w-3" /> : <ArrowDown className="h-3 w-3" />)}
                                                </div>
                                            </DropdownMenuItem>
                                        </>
                                    )}
                                    ...
                                    <DropdownMenuItem
                                        onClick={() => {
                                            setSearch('');
                                            router.get('/teacher/assignments', filters?.subject_id ? { subject_id: filters.subject_id } : {}, { replace: true });
                                        }}
                        ...
                        {mode === 'assignments' && selectedSubject && (
                            <Button className="h-10 w-full gap-2 shadow-lg shadow-primary/20 sm:w-auto" asChild>
                                <Link href={`/teacher/assignments/create?subject_id=${selectedSubject?.id}`}>
                            ...
                            <table className="w-full border-collapse text-left text-sm">
                                <thead>
                                    <tr className="border-b border-zinc-200 bg-muted/50 dark:border-zinc-800">
                                        <th className="p-4 font-bold text-muted-foreground uppercase tracking-wider">Judul Tugas</th>
                                        <th className="p-4 font-bold text-muted-foreground uppercase tracking-wider">Status</th>
                                        <th className="p-4 font-bold text-muted-foreground uppercase tracking-wider">Tenggat & Nilai</th>
                                        <th className="p-4 font-bold text-muted-foreground uppercase tracking-wider">Pengumpulan</th>
                                ...
                                <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
                                    {assignments && assignments?.data?.length > 0 ? (
                                                ...
                                                <td className="p-4">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div className="flex flex-col min-w-0">
                                                            <span className="font-semibold truncate group-hover:text-primary transition-colors">{assignment?.title}</span>
                                                            <span className="text-xs text-muted-foreground truncate max-w-[300px]">{assignment?.description || 'Tanpa petunjuk deskripsi'}</span>
                                                        </div>
                                                    </div>
                                                </td>
                                                ...
                                                <td className="p-4 text-xs">
                                                    <div className="flex items-center gap-3">
                                                        <span className="flex items-center gap-1">
                                                            ...
                                                            {assignment.due_date ? new Date(assignment.due_date).toLocaleString('id-ID', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : 'Tanpa tenggat'}
                                                        </span>
                                                    ...
                                                    <div onClick={(e) => e.stopPropagation()}>
                                                        <DropdownMenu>
                                                            ...
                                                            <DropdownMenuContent align="end" className="w-44">
                                                                <DropdownMenuItem asChild><Link href={`/teacher/assignments/${assignment.id}`}><Eye className="mr-2 h-4 w-4 text-primary" /> Detail & Nilai</Link></DropdownMenuItem>
                                                                {auth?.user?.role === 'guru' && (
                                                                    <>
                                                                        <DropdownMenuItem asChild><Link href={`/teacher/assignments/${assignment.id}/edit`}><Pencil className="mr-2 h-4 w-4 text-primary" /> Edit Tugas</Link></DropdownMenuItem>
                                                                        <DropdownMenuItem className="text-destructive focus:text-destructive" onSelect={() => setAssignmentToDelete(assignment)}><Trash2 className="mr-2 h-4 w-4" /> Hapus</DropdownMenuItem>
                                                                    </>
                                                                )}
                                                            </DropdownMenuContent>
                                                        </DropdownMenu>
                                        ...
                                        <tr><td colSpan={5} className="p-12 text-center text-muted-foreground italic">Belum ada tugas pada mata pelajaran ini.</td></tr>
                                    )}
                                </tbody>
                            </table>
                        </div>
                        {assignments && renderPagination(assignments)}
                    </Card>
                )}
                ...
                <Dialog open={!!assignmentToDelete} onOpenChange={(open) => !open && setAssignmentToDelete(null)}>
                    <DialogContent className="sm:max-w-[425px]">
                        <DialogHeader>
                            ...
                            <DialogDescription>Apakah Anda yakin ingin menghapus tugas <span className="font-bold text-foreground">{assignmentToDelete?.title}</span>? Seluruh berkas pengumpulan siswa untuk tugas ini juga akan terhapus.</DialogDescription>
                        </DialogHeader>
                        <DialogFooter className="mt-4 gap-2 sm:gap-0">
                            <Button variant="ghost" onClick={() => setAssignmentToDelete(null)} disabled={isDeleting}>Batal</Button>
